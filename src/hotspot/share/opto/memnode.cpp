...
static Node *step_through_mergemem(PhaseGVN *phase, MergeMemNode *mmem,  const TypePtr *tp, const TypePtr *adr_check, outputStream *st) {
  ...
  if (tp->base() != Type::AnyPtr &&
    ...
    // The current method may be called with phase->_delay_transform set to true
    // and if that happens and mmem is already a registered node then validation
    // inside transform will complain.
    Node* m  = mmem;
    PhaseIterGVN* igvn = phase->is_IterGVN();
